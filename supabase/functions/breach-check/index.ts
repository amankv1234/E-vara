...
serve(async (req) => {
  ...
  try {
    ...
    const { data: limitAllowed, error: rpcError } = await supabase.rpc('check_rate_limit', {
      p_user_id: user.id,
    ...
    if (jobError) {
      console.error("Queue insertion failed:", jobError);
      throw new Error('ERR_QUEUE_FAILED');
    }

    return new Response(
      JSON.stringify({ success: true, status: "JOB_QUEUED", message: "Scan dispatched to distributed queue." }),
