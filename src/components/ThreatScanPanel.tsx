...
const ThreatScanPanel = () => {
  ...
  const metrics = useMemo(() => {
    ...
    const barData = Object.entries(sourceMap).map(([name, value]) => ({ name, value }));
  ...
  if (error) {
    return (
      <section className="rounded-xl border border-[hsl(var(--severity-high)/0.4)] bg-[hsl(var(--severity-high)/0.05)] p-6 flex flex-col items-center justify-center text-center h-48">
        ...
        <h3 className="text-sm font-semibold text-[hsl(var(--severity-high))]">Scan Terminated</h3>
        <p className="text-xs text-[hsl(var(--severity-high))/80] mt-1">{error.message}</p>
      </section>
    );
  }
    ...
    return (
      <section className="rounded-xl border border-[#244163] bg-[#0a1224] p-6 flex items-center justify-center h-48">
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm font-mono text-primary animate-pulse">Initializing Scan Engines...</p>
        </div>
      </section>
    );
  }
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <ChartCard title="Risk Distribution">
              <div className="h-56">
                <ResponsiveContainer>
                  <PieChart>
                    <Pie data={pieData} dataKey="value" nameKey="name" outerRadius={84} innerRadius={46}>
                      {pieData.map((entry) => <Cell key={entry.name} fill={entry.color} />)}
                    </Pie>
                    <Tooltip 
                      contentStyle={{ backgroundColor: '#0E1015', border: '1px solid #ffffff10', fontSize: '12px' }}
              ...
              <div className="h-56">
                <ResponsiveContainer>
                  <BarChart data={barData}>
                    <XAxis dataKey="name" stroke="#8eb3d6" fontSize={10} tickLine={false} />
                    <YAxis stroke="#8eb3d6" fontSize={10} tickLine={false} allowDecimals={false} />
                    <Tooltip 
                      contentStyle={{ backgroundColor: '#0E1015', border: '1px solid #ffffff10', fontSize: '12px' }}
                      cursor={{ fill: '#ffffff05' }}
                    />
                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                      {barData.map((_, i) => <Cell key={i} fill={["#00e5ff", "#3da4ff", "#31d3c6", "#ff7c53"][i % 4]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
