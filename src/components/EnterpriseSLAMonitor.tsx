    ...
    <div className="neon-panel rounded-xl border border-primary/20 p-5 bg-[#0A0C10]/80 backdrop-blur-md">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">System Health: Optimal</span>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {systems.map((system) => (
          <div key={system.name} className="p-4 rounded-lg border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-mono uppercase text-muted-foreground">{system.name}</span>
            ...
            <div className="flex flex-col">
              <span className={`text-lg font-bold font-mono ${system.status === 'Operational' ? 'text-foreground' : 'text-warning'}`}>
                {system.status}
              </span>
            ...
            <div className="mt-3 h-1 w-full bg-white/5 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: system.status === 'Operational' ? '100%' : '70%' }} 
              />
            </div>
          </div>
        ))}
      ...
      <div className="mt-6 p-4 rounded-lg border border-primary/10 bg-primary/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-foreground">Global Intelligence Network</p>
            <p className="text-[10px] text-muted-foreground">Active nodes in 12 regions. Real-time synchronization enabled.</p>
          </div>
        </div>
